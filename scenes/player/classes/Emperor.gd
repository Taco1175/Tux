...
func _use_secondary_ability() -> void:
	...
	if multiplayer.is_server():
		_request_power_chord(global_position, aim_dir, beat_bonus)
	else:
		_request_power_chord.rpc_id(1, global_position, aim_dir, beat_bonus)


@rpc("any_peer", "reliable")
func _request_power_chord(origin: Vector2, direction: Vector2, beat_bonus: float = 1.0) -> void:
	if not multiplayer.is_server():
		return
	var dmg := int(POWER_CHORD_DAMAGE * clampf(beat_bonus, 1.0, 1.25))
	...
	for enemy in get_tree().get_nodes_in_group("enemies"):
		...
		if dist <= POWER_CHORD_RANGE:
			if dist < 1.0 or to_enemy.normalized().dot(direction) > 0.2:
				(enemy as CharacterBody2D).take_damage(dmg)
...
func take_damage(amount: int, attacker: Node2D = null) -> void:
	if randf() < block_chance:
		...
		var beat_info := BeatClock.is_on_beat()
		if beat_info.on_beat:
			MusicManager.add_intensity(0.12)
